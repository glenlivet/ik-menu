	(function($){
		...
		$.widget('ik.menu',{
			...
			_handleTopData: function(data, $topOL){
				...
				//add target to data
				data.target = topLI[0];
				this._targetsMap[data.id]= topLI[0];
				//CLICK EVENT
				aTag.click(function(){
					if(hasChildren){
						...
						if(that._isOpenedSet(data.target)){
							...
							var index = that._openedSet.indexOf(data.target);
							if(index !== -1){
			...
			selectItem: function(target){
				if(typeof target == 'object'){
					if($(target).hasClass('ik-menu-set-li')){
						var topLiEle = $(target).parent().parent()[0];
						this._openSet(topLiEle);
						this._selectSetItem($(target).children('a')[0]);
					}else if($(target).hasClass('ik-menu-top-li')){
						this._selectTopLabel($(target).children('a')[0]);
					}else {
						throw new Error('Illegal Argument in method \'selectItem\'');
					}
				}else {
					var t = this._targetsMap[target];
					this.selectItem(t);
				}
			},
